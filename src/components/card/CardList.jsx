...
function CardList({ itemList }) {
    ...
    const isNotHome = location.pathname === "/product" || "/artist" ? false : true;
    console.log("isHome", isNotHome);
    const goDetail = (id, price) => {
        nagigate(price ? `/product/${id}` : `/artist/${id}`)
    }
    return (
        <>
            {isNotHome && <h1 className={style.responsiveTitle}>{routeCheck(location.pathname)} List</h1>}
            ...
            <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 h-auto">
                {itemList.map((item, index) => (
